#!/usr/bin/env python3
"""
Fixed Natural Semantic Segmentation Training v2
===============================================

Corrected version that uses RGB_color_image_masks directory for proper RGB-encoded labels.
"""
...
def validate_dataset_v2(data_root: str):
    """Validate dataset using RGB_color_image_masks directory."""
    data_root = Path(data_root)
    image_dir = data_root / "original_images"
    
    # Use RGB_color_image_masks instead of label_images_semantic
    label_dir = data_root.parent.parent / "RGB_color_image_masks" / "RGB_color_image_masks"
    
    print(f"Validating dataset in {data_root}")
    print(f"Image dir: {image_dir}")
    print(f"Label dir (RGB masks): {label_dir}")
    ...
    for img_path in tqdm(image_files, desc="Validating dataset"):
        # Check corresponding RGB mask file - both should be .png for RGB masks
...
class FixedNaturalSemanticDatasetV2(torch.utils.data.Dataset):
    """Fixed dataset that uses RGB color masks for proper semantic segmentation."""
    ...
    def rgb_to_class_index(self, rgb_label):
        """Convert RGB label to class indices with tolerance for slight variations."""
        h, w, c = rgb_label.shape
        class_label = np.zeros((h, w), dtype=np.uint8)
        
        # Reshape for easier processing
        rgb_pixels = rgb_label.reshape(-1, 3)
        class_pixels = np.zeros(rgb_pixels.shape[0], dtype=np.uint8)
        
        # Use tolerance-based matching for robustness
        for rgb, class_idx in RGB_TO_CLASS.items():
            rgb_array = np.array(rgb)
            # Calculate L2 distance to handle slight variations in RGB values
            distances = np.sqrt(np.sum((rgb_pixels - rgb_array) ** 2, axis=1))
            mask = distances < 10  # Allow small tolerance
            class_pixels[mask] = class_idx
            
        class_label = class_pixels.reshape(h, w)
        return class_label
...
def main():
    ...
    print("Step 1: Validating dataset (using RGB color masks)...")
    try:
        valid_pairs = validate_dataset_v2(args.data_root)
    except Exception as e:
    ...
    # Initialize wandb
    wandb.init(
        project="uav-natural-semantic-rgb-fixed",
        config=vars(args),
        name=f"natural_24class_rgb_fixed_{args.image_size}px"
    )
    ...
    train_dataset = FixedNaturalSemanticDatasetV2(valid_pairs, 'train', args.image_size)
    val_dataset = FixedNaturalSemanticDatasetV2(valid_pairs, 'val', args.image_size)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_accuracy > best_accuracy:
            ...
            torch.save(checkpoint, 'outputs/natural_semantic_rgb_best_v2.pth')
            print(f"New best model saved! Accuracy: {best_accuracy:.4f}")
    
    print(f"Training completed! Best accuracy: {best_accuracy:.4f}")
    print(f"Model saved to: outputs/natural_semantic_rgb_best_v2.pth")
