...
class FastMobileNet(nn.Module):
    """
    Even faster variant for extreme edge constraints.
    
    Uses MobileNetV3-Small with further optimizations:
    - Reduced feature channels
    - Single-scale processing
    - Minimal post-processing
    """
    
    def __init__(self, num_classes: int = 6, width_mult: float = 0.75):
        super(FastMobileNet, self).__init__()
        
        self.num_classes = num_classes
        
        # Ultra-light backbone
        self.backbone = mobilenet_v3_small(pretrained=True)
        
        # Reduce feature channels
        backbone_channels = int(576 * width_mult)
        
        # Minimal segmentation head
        self.classifier = nn.Sequential(
            nn.AdaptiveAvgPool2d((8, 8)),  # Global context
            nn.Conv2d(576, backbone_channels, 1, bias=False),
...
class QuantizedEdgeNet(nn.Module):
    """
    Quantization-ready version for deployment.
    
    Designed for INT8 quantization with minimal accuracy loss.
    """
    
    def __init__(self, base_model: EdgeLandingNet):
        super(QuantizedEdgeNet, self).__init__()
...
def create_edge_model(
    ...
    **kwargs
    ...
    if model_type == "standard":
        ...
        return FastMobileNet(
            num_classes=num_classes,
            **kwargs
        )
    
    elif model_type == "quantized":
        base_model = EdgeLandingNet(
            num_classes=num_classes,
            input_size=input_size,
            **kwargs
        )
        return QuantizedEdgeNet(base_model)
...
if __name__ == "__main__":
    ...
    # Test fast model
    fast_model = create_edge_model("fast", num_classes=6)
    fast_cpu_stats = benchmark_model(fast_model, device='cpu', num_runs=50)
    print(f"\nFast Model CPU: {fast_cpu_stats['mean_ms']:.1f}±{fast_cpu_stats['std_ms']:.1f}ms")
