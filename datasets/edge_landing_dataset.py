...
class EdgeLandingDataset(Dataset):
    ...
    def _load_semantic_drone(self, dataset_path: Path, config: Dict):
        ...
        # Original 24-class to 6-class mapping (optimized for landing)
        semantic_to_landing = {
    ...
    def _load_udd6(self, dataset_path: Path, config: Dict):
        ...
        udd_to_landing = {
            ...
            1: 3,    # Facade → obstacle
            2: 1,    # Road → safe_flat
            3: 2,    # Vegetation → safe_soft
            4: 3,    # Vehicle → obstacle
            5: 3,    # Roof → obstacle
        }
    ...
    def _load_drone_deploy(self, dataset_path: Path, config: Dict):
        ...
        # DroneDeploy grayscale values to 6-class mapping
        dronedeploy_to_landing = {
            81: 3,   # Building → obstacle
            91: 1,   # Road → safe_flat
            99: 3,   # Car → obstacle
            105: 0,  # Background → unknown
            132: 2,  # Trees → safe_soft
        ...
        image_files = sorted(list(images_dir.glob("*.tif")))[:20]  # Limit to manageable size
    ...
    def get_class_weights(self) -> torch.Tensor:
        ...
        # Apply safety multipliers (match your successful approach)
